...
describe('kadenaGetPublic', () => {
    ...
    it('should retrieve distinct public keys from seedBuffer for different indexes', async () => {
        ...
        const publicKeys = await Promise.all(indexes.map((index) => kadenaGetPublic(password, seed, index)));
    ...
    it('should get the similar public keys as Enkrypt for the same path', async () => {
        const password = 'pass';
        const seed = await kadenaMnemonicToSeed(password, 
        ...
        let publicKey = await kadenaGetPublic(password, seed, 0, "m'/44'/626'/<index>/0'/0'");
        expect(publicKey).toBe('43726c4a2e7b03fa5d23635307e5b130baf8b261e1081c099a2b43db1d4554cc');
        publicKey = await kadenaGetPublic(password, seed, 1, "m'/44'/626'/<index>/0'/0'");
        expect(publicKey).toBe('3f53dfad097fdf8501c32b275e109980ed7121866a63ca34bb035c4a2e41a265');
        publicKey = await kadenaGetPublic(password, seed, 2, "m'/44'/626'/<index>/0'/0'");
        expect(publicKey).toBe('3021bcfa703cc4fac007ab4c5050df5c0b8ca7d655ea80c84af9ea5e43ecf0ff');
    });
});
