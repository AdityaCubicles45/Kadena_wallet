...
import { kadenaGenKeypairFromSeed, kadenaGenMnemonic, kadenaMnemonicToSeed, } from '../index.js';
...
describe('kadenaGenKeypairFromSeed', () => {
    it('should generate an encrypted keypair from the seedBuffer when a password is provided', async () => {
        ...
        const [publicKey, encryptedPrivateKey] = await kadenaGenKeypairFromSeed(password, seed, 0);
    ...
    it('should generate a range of keypairs from the seed', async () => {
        ...
        await Promise.all(keyPairs.map(async ([publicKey, privateKey]) => {
            expect(publicKey).toHaveLength(64);
            expect(Buffer.from(await kadenaDecrypt(password, privateKey)).toString('hex')).toHaveLength(64);
        }));
    });
    it('should throw an error for out-of-bounds index values', async () => {
        ...
        await expect(() => kadenaGenKeypairFromSeed(password, seed, outOfBoundsIndex)).rejects.toThrowError('Invalid child index: -1');
    });
    it('returns an encrypted private key that can be decrypted with the password', async () => {
        const mnemonic = kadenaGenMnemonic();
        const password = 'password';
        const seed = await kadenaMnemonicToSeed(password, mnemonic);
        const [, encryptedPrivateKey] = await kadenaGenKeypairFromSeed(password, seed, 0);
        const decryptedPrivateKey = await kadenaDecrypt(password, encryptedPrivateKey);
