...
import { kadenaChangePassword, kadenaDecrypt, kadenaEncrypt, } from '../kadenaEncryption.js';
describe('kadenaDecrypt', () => {
    it('should correctly decrypt a previously encrypted string', async () => {
        ...
        expect(Buffer.from(decryptedMessage).toString('utf-8')).toEqual(message.toString('utf-8'));
    ...
    it('should throw an error when the incorrect password is provided', async () => {
        ...
        await expect(() => kadenaDecrypt(wrongPassword, encryptedMessage)).rejects.toThrow('Decryption failed');
    });
    it('should throw an error if the encrypted key is corrupted', async () => {
        const password = 'test-password';
        const corruptedEncryptedPrivateKey = 'corrupted-data';
        await expect(() => kadenaDecrypt(password, corruptedEncryptedPrivateKey)).rejects.toThrow();
    });
    it('should throw an error if the encrypted data is empty', async () => {
        const password = 'test-password';
        const emptyEncryptedMessage = '';
        await expect(() => kadenaDecrypt(password, emptyEncryptedMessage)).rejects.toThrow('Encrypted data is empty');
    });
    it('should handle passwords with special characters', async () => {
        const specialCharPassword = 'p@ssw0rd!#%&';
        const message = Buffer.from('test-message');
        const encryptedMessage = await kadenaEncrypt(specialCharPassword, message);
        const decryptedMessage = await kadenaDecrypt(specialCharPassword, encryptedMessage);
    ...
    it('should handle extremely long passwords', async () => {
        ...
        const decryptedMessage = await kadenaDecrypt(longPassword, encryptedMessage);
        expect(Buffer.from(decryptedMessage).toString()).toBe(message.toString());
    });
    it('should handle unicode characters in passwords', async () => {
        ...
        const decryptedMessage = await kadenaDecrypt(unicodePassword, encryptedMessage);
        expect(Buffer.from(decryptedMessage).toString()).toBe(message.toString());
    });
});
describe('kadenaChangePassword', () => {
    ...
    it('changes the password successfully and allows decryption with the new password', async () => {
        ...
        const newEncryptedMessage = await kadenaChangePassword(firstPassword, encryptedMessage, secondPassword);
        expect(encryptedMessage.toString()).not.toBe(newEncryptedMessage.toString());
        const decryptedMessage = Buffer.from(await kadenaDecrypt(secondPassword, newEncryptedMessage));
    ...
    it('throws an error when the password is incorrect', async () => {
        ...
        const changePasswordAttempt = () => kadenaChangePassword(incorrectPassword, encryptedPrivateKey, newPassword);
        await expect(() => changePasswordAttempt()).rejects.toThrow('Failed to decrypt the private key with the old password: Decryption failed');
    });
    it('fails to decrypt with the old password after the password has been changed', async () => {
        ...
        const encryptedMessage = await kadenaEncrypt(firstPassword, message);
        const newEncryptedMessage = await kadenaChangePassword(firstPassword, encryptedMessage, secondPassword);
        await expect(() => kadenaDecrypt(firstPassword, newEncryptedMessage)).rejects.toThrow();
    });
    ...
    it('handles non-string inputs for private keys and passwords', async () => {
        await expect(() => kadenaChangePassword(password, testUndefined, newPassword)).rejects.toThrow();
        await expect(() => kadenaChangePassword(testUndefined, testMessage, newPassword)).rejects.toThrow();
        await expect(() => kadenaChangePassword(password, testMessage, testUndefined)).rejects.toThrow();
        await expect(() => kadenaChangePassword(password, testNull, newPassword)).rejects.toThrow();
        await expect(() => kadenaChangePassword(testNull, testUnint8Array, newPassword)).rejects.toThrow();
        await expect(() => kadenaChangePassword(password, testMessage, testNull)).rejects.toThrow();
        await expect(() => kadenaChangePassword(password, testMessage, password)).rejects.toThrow();
    });
});
