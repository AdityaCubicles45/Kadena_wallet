 ...
 * @public
 */
export async function kadenaEncrypt(password, message, encode = 'base64') {
    // Using randomBytes for the salt is fine here because the salt is not secret but should be unique.
    const salt = randomBytes(16);
    const { cipherText, iv } = await encrypt(typeof message === 'string' ? Buffer.from(message) : message, password, salt);
 ...
 * @public
 */
export async function kadenaDecrypt(password, encryptedData) {
    ...
    const decrypted = await decrypt({ cipherText, iv }, password, salt).catch(() => undefined);
    if (decrypted)
 ...
 * @public
 */
export async function kadenaChangePassword(password, encryptedData, newPassword, encode = 'base64') {
